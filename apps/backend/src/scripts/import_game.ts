...
if (!GITHUB_URL) {
  console.error(
    'Usage: bun run src/scripts/import_game.ts <GITHUB_URL> [--manifest PATH] [--branch BRANCH]'
  );
  console.error(
    'Example: bun run src/scripts/import_game.ts https://github.com/Ryan-DPC/Vext-platform --manifest games/aether_strike/vext.json --branch dev'
  );
  process.exit(1);
}
...
const main = async () => {
  try {
    ...
    console.log(
      `📦 Fetching metadata for ${repoInfo.owner}/${repoInfo.repo} (Manifest: ${MANIFEST_PATH}, Branch: ${BRANCH})...`
    );

    // 1. Fetch vext.json
    const manifestBuffer = await ghService.getRawFile(
      repoInfo.owner,
      repoInfo.repo,
      MANIFEST_PATH,
      BRANCH
    ...
    if (!manifestBuffer) {
      throw new Error(
        `${MANIFEST_PATH} not found in repository on branch '${BRANCH}'. This file is required for Vext import.`
      );
    }
    ...
    if (release) {
      ...
      console.warn(
        '⚠️ No releases found on GitHub. Game will be imported but not downloadable yet.'
      );
    }
    ...
    const game = await GameModel.findOneAndUpdate({ folder_name: manifest.folder_name }, gameData, {
      upsert: true,
      new: true,
    });
    ...
    if (ADMIN_SECRET) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.warn(
            `⚠️ Failed to clear remote cache: ${response.status} ${response.statusText}`
          );
