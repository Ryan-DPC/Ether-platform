...
export default class Games {
  ...
  static async updateGameVersion(
    gameId: string,
    version: string,
    manifestUrl: string,
    zipUrl: string
    ...
    const result = await GameModel.updateOne(query, {
      $set: {
        version: version,
  ...
  static async getGamesByKeysOrIds(keys: string[], ids: string[]): Promise<any[]> {
    return await GameModel.find({
      $or: [{ folder_name: { $in: keys } }, { _id: { $in: ids } }],
    }).lean();
  }

  static async updateGameMetadata(folderName: string, updates: any): Promise<boolean> {
    const result = await GameModel.updateOne({ folder_name: folderName }, { $set: updates });
    return result.modifiedCount > 0;
  }
