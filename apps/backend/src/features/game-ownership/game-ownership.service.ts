...
export class GameOwnershipService {
  ...
  static async _populateGameDetails(ownershipDocs: any[]): Promise<any[]> {
    ...
    const gameIds = [
      ...new Set(ownershipDocs.map((d) => d.game_id || d.gameId).filter((id) => id)),
    ];
  ...
  static async listForSale(
    ...
    if (!ownership.game_name) {
      const gameDetails =
        (await Games.getGameById(String(ownership.game_id))) ||
        (await Games.getGameByName(ownership.game_key));
   ...
   */
  static async purchaseUsedGame(
    buyerId: string,
    ownershipToken: string,
    sellerId: string
  ): Promise<any> {
    throw new Error(
      'This endpoint is deprecated. Please use WebSocket transaction:purchase event.'
    );
  }
  ...
  static async getTransactions(userId: string): Promise<any[]> {
    ...
    const transactions = await BlockchainTxModel.find({
      $or: [{ from_address: userId }, { to_address: userId }],
    })
      .sort({ timestamp: -1 })
      .limit(50)
      .lean();
  ...
  static async deleteListing(userId: string, ownershipToken: string): Promise<any> {
    ...
    if (!isOwner && !isAdmin) {
      throw new Error(
        'Unauthorized: You can only delete your own listings unless you are an admin'
      );
    }
