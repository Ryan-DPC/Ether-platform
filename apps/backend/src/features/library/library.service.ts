...
const GAMES_LIBRARY_PATH =
  process.env.GAMES_LIBRARY_PATH || path.resolve(process.cwd(), '../../../../games');
...
export class LibraryService {
  ...
  async purchaseGame(userId: string, gameId: string | any) {
    ...
    try {
      ...
      console.warn(
        'Transactions not supported (standalone Mongo?), continuing without transaction safety.'
      );
      session = null;
    }
    ...
    try {
      ...
      if (!game && folderName) {
        logger.info(
          `[Library] Game ${folderName} not found in MongoDB, attempting Cloudinary creation...`
        );
        try {
          const cloudinaryService = new CloudinaryService();
          if (cloudinaryService.isEnabled()) {
            const manifestUrl = cloudinaryService.getPublicUrl(
              `games/dev/${folderName}/manifest.json`,
              'raw'
            );
            try {
              if (manifestUrl) {
                ...
                if (response.ok) {
                  ...
                  const gameData = {
                    ...
                    image_url:
                      cloudinaryService.getPublicUrl(
                        `games/dev/${folderName}/image.png`,
                        'image'
                  ...
                  const cloudinaryGame: any = cloudinaryGames.find(
                    (g: any) => g.folder_name === folderName
                  );
                  if (cloudinaryGame) {
                    const gameData = {
          ...
          console.warn(
            `[Library] ⚠️ Impossible to create game from Cloudinary:`,
      ...
      if (availableKey) {
        ...
        const updateQuery = GameKey.updateOne(
      ...
      if (gamePriceCHF > 0 && userCHF < gamePriceCHF) {
        throw new Error(
          `Solde CHF insuffisant. Vous avez ${userCHF.toFixed(2)} CHF, prix: ${gamePriceCHF.toFixed(2)} CHF`
        );
      }
  ...
  async purchaseUsedGame(buyerId: string, gameKey: string, sellerId: string) {
    try {
      ...
      if (!buyer || buyerCHF < askingPrice) {
        throw new Error(
          `Solde CHF insuffisant. Vous avez ${buyerCHF.toFixed(2)} CHF, prix: ${askingPrice.toFixed(2)} CHF`
        );
      }
  ...
  async installGame(userId: string, gameKey: string) {
    try {
      ...
      if (gameFolderName) {
        ...
        try {
          if (
            NpmInstaller.needsNpmInstall(gameFolderPath) &&
            !NpmInstaller.areDependenciesInstalled(gameFolderPath)
          ) {
            console.log(
              `[Library] Installation automatique des dépendances pour ${gameFolderName}...`
            );
            await NpmInstaller.installDependencies(gameFolderPath);
          }
        } catch (npmError: any) {
          console.warn(
            `[Library] ⚠️ Impossible d'installer les dépendances automatiquement:`,
            npmError.message
          );
        }
      }
          ...
          try {
            ...
            if (fs.existsSync(gameManifestPath)) {
              ...
              if (manifest.platform === 'exe' || manifest.entryPoint?.endsWith('.exe')) {
                gameType = 'exe';
                if (r.game_type !== 'exe') {
                  GameOwnership.updateOne({ _id: r._id }, { $set: { game_type: 'exe' } }).catch(
                    (err: any) =>
                      console.warn(
                        `[Library] Erreur lors de la mise à jour du game_type pour ${folderName}:`,
                        err.message
                      )
                  );
                }
  ...
  async getMarketplaceGames(filters: any = {}, excludeUserId: string | null = null) {
    try {
      ...
      const filtered = rows.filter((r: any) =>
        genre ? (r.game_id?.genre || '').toLowerCase() === String(genre).toLowerCase() : true
      );
  ...
  async getUserTransactionHistory(userId: string) {
    try {
      const userAddress = `user_${userId}`;
      const rows = await BlockchainTx.find({
        $or: [{ from_address: userAddress }, { to_address: userAddress }],
      })
        .sort({ timestamp: -1 })
  ...
  getBlockchainStats() {
    return {
      ...
      totalTransactions: this.blockchain.chain.reduce(
        (total: number, block: any) => total + block.transactions.length,
        0
      ),
  ...
  async generateKeys(
    ...
    try {
      ...
      for (let i = 0; i < quantity; i++) {
        ...
        // Check initial
        let exists =
          (await GameOwnership.findOne({ game_key: keyFinal })) ||
          (await GameKey.findOne({ key: keyFinal }));
        while (exists) {
