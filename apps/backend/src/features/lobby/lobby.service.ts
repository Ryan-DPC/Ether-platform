...
export class LobbyService {
    ...
    static async createSession(userId: string, gameId: string, gameFolderName: string, ownershipToken: string) {
        try {
            const activeSession = await this.getActiveSession(userId);
            if (activeSession) {
                throw new Error(`Un jeu est déjà en cours: ${activeSession.game_folder_name}. Veuillez le fermer.`);
            }
