  ...
  static async installDependencies(gameFolderPath: string): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      if (!fs.existsSync(packageJsonPath)) {
        console.log(
          `[NpmInstaller] Pas de package.json trouvé dans ${gameFolderPath}, aucune dépendance à installer`
        );
        return resolve({
          success: true,
      ...
      if (fs.existsSync(nodeModulesPath)) {
        console.log(
          `[NpmInstaller] node_modules existe déjà dans ${gameFolderPath}, installation ignorée`
        );
        return resolve({
          success: true,
      ...
      npmProcess.on('close', (code) => {
        if (code === 0) {
          console.log(
            `[NpmInstaller] ✅ Dépendances installées avec succès dans ${gameFolderPath}`
          );
          resolve({
            success: true,
          ...
          error.stderr = stderr;
          console.error(
            `[NpmInstaller] ❌ Erreur lors de l'installation des dépendances:`,
            error.message
          );
          reject(error);
        }
