...
export class CloudinaryService {
    ...
    async listGamesMetadata(clearCache = false) {
        ...
        try {
            ...
            const gamesMetadata = await Promise.all(metadataFiles.map(async (resource: any) => {
                try {
                    ...
                    console.error(`[Cloudinary] Error fetching metadata for ${resource.public_id}: ${err.message}`);
                    return null;
                }
            }));
